...
export function CloudModels({ className }: CloudModelsProps) {
  ...
  const fetchProviders = async () => {
    ...
    setError(null);
    try {
  const response = await fetch((import.meta.env.VITE_API_URL || (import.meta.env.MODE === 'production' ? 'https://stocks-with-ai.onrender.com' : 'http://localhost:8000')) + '/language-models/providers');
      if (response.ok) {
